jQuery(document).ready(function ($) {
	...
	$(".page_collapsible").collapsible({
		...
		animateOpen: function (elem, opts) {
			...
			$(".collapse-close")
				.find("span")
				.removeClass("fa-arrow-circle-o-right block-indicator");
			elem.find("span").addClass(
				"fa-arrow-circle-o-right block-indicator"
			);
			$(".wcfm-tabWrap")
				.find(".wcfm-container")
				.stop(true, true)
		...
		animateClose: function (elem, opts) {
			elem.find("span").removeClass(
				"fa-arrow-circle-o-up block-indicator"
			);
	...
	if ($("#associate_products").length > 0) {
		$("#associate_products")
			.find(".associate_product")
			.select2($wcfm_simple_product_select_args);
	}

	$("#associate_products")
		.find(".add_multi_input_block")
		.click(function () {
			$("#associate_products")
				.find(".multi_input_block:last")
				.find(".associate_product")
				.val("")
				.select2($wcfm_simple_product_select_args);
			$("#associate_products")
				.find(".multi_input_block:last")
				.find(".associate_product_variation")
				.html("")
				.addClass("wcfm_ele_hide");
			$("#associate_products")
				.find(".multi_input_block:last")
				.find(".associate_product_variation_label")
				.addClass("wcfm_ele_hide");
			$("#associate_products")
				.find(".multi_input_block:last")
				.find(".associate_product_qty")
				.val("1");
			variationSelectProperty(
				$("#associate_products").find(".associate_product")
			);
		});
			...
			if ($selected_product) {
				jQuery.each($wcfm_search_products_list, function (id, product) {
					if ($selected_product == id) {
						...
						$associate_product
							.parent()
							.find(".associate_product_price")
							.val(parseFloat(product.price));
						if (!jQuery.isEmptyObject($variations)) {
							$.each(
								$variations,
								function ($variation_id, $variation_label) {
									$variations_html +=
										'<option value="' +
										$variation_id +
										'">' +
							...
							$associate_product
								.parent()
								.find(".associate_product_variation")
								.html($variations_html)
								.removeClass("wcfm_ele_hide");
							$associate_product
								.parent()
								.find(".associate_product_variation_label")
								.removeClass("wcfm_ele_hide");
	...
	$("#sadd_as_billing")
		.on("change", function () {
			...
			resetCollapsHeight($(this).parent());
		})
		.change();

	$(".wcfm_order_add_new_customer_box .wcfm_order_add_new_customer").click(
		function (event) {
				...
				success: function (response) {
					...
					jQuery.colorbox({
						...
						onComplete: function () {
							...
							$("#wcfm_order_add_customer_button").click(
								function () {
									...
									if (!$wcfm_is_valid_form) {
										...
										$(
											".wcfm_order_add_customer_button"
										).hide();
										var data = {
											action: "wcfm_ajax_controller",
											controller:
												"wcfm-orders-manage-add-customer",
											wcfm_order_add_customer_form: $(
												"#wcfm_order_add_customer_form"
											).serialize(),
											wcfm_ajax_nonce:
											...
											function (response) {
												if (response) {
													...
													$(".wcfm-message")
														...
														.slideUp();
													if (
														jQueryresponse_json.status
													) {
														$(
															"#wcfm_order_add_customer_form .wcfm-message"
														)
															.html(
																'<span class="wcicon-status-completed"></span>' +
																	jQueryresponse_json.message
															)
															.addClass(
																"wcfm-success"
															)
														...
														if (
															...
															$(
																...
																.append(
																	'<option value="' +
																		jQueryresponse_json.customer_id +
																		'" selected>#' +
																		jQueryresponse_json.customer_id +
																		" " +
																		jQueryresponse_json.username +
																		"</option>"
																);
															$(
																"#wcfm_orders_manage_expander"
	...
	function wcfm_orders_manage_form_validate() {
		...
		var associate_product = $.trim(
			$("#wcfm_orders_manage_form")
				.find("#associate_products")
				.find(".associate_product:first")
				.val()
		);
		if (associate_product.length == 0) {
			$is_valid = false;
			$("#wcfm_orders_manage_form .wcfm-message")
				.html(
					'<span class="wcicon-status-cancelled"></span>' +
						wcfm_orders_manage_messages.no_product
				)
				.addClass("wcfm-error")
				.slideDown();
			audio.play();
		...
		if ($is_valid) {
			...
			$.post(wcfm_params.ajax_url, data, function (response) {
				if (response) {
					...
					if ($response_json.id)
						$("#customer_id").val($response_json.id);
					wcfmMessageHide();

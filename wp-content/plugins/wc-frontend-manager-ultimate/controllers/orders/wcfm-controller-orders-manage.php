...
class WCFMu_Orders_Manage_Controller
{
    ...
    public function processing()
    {
        ...
        if (!isset($wcfm_orders_manage_form_data['wcfm_invoice_number']) || empty($wcfm_orders_manage_form_data['wcfm_invoice_number'])) {
            echo '{"status": false, "message": "Debe ingresar el Número Comprobante"}';
            die;
        }
        ...
        if (isset($wcfm_orders_manage_form_data['associate_products']) && ! empty($wcfm_orders_manage_form_data['associate_products'])) {
            ...
            try {
                ...
                foreach ($associate_products as $associate_product) {
                    ...
                    if ($product_id) {
                        ...
                        $total = wc_get_price_excluding_tax($product, [ 'qty' => $quantity, 'price' => is_numeric($associate_product['price']) ? $associate_product['price'] : '' ]);
                ...
                if (isset($wcfm_orders_manage_form_data['wcfm_om_shipping_method']) && ! empty($wcfm_orders_manage_form_data['wcfm_om_shipping_method'])) {
                    $shipping_rate = new WC_Shipping_Rate($wcfm_orders_manage_form_data['wcfm_om_shipping_method'], '', isset($wcfm_orders_manage_form_data['wcfm_om_shipping_cost']) ? floatval($wcfm_orders_manage_form_data['wcfm_om_shipping_cost']) : 0, [], $wcfm_orders_manage_form_data['wcfm_om_shipping_method']);
                ...
                if (wc_coupons_enabled() && apply_filters('wcfm_orders_manage_discount', true) && isset($wcfm_orders_manage_form_data['wcfm_om_discount']) && ! empty($wcfm_orders_manage_form_data['wcfm_om_discount'])) {
                    ...
                    // if (! is_wp_error($new_coupon_id)) {
                        ...
                        try {
                            $result = $order->apply_coupon($wcfm_orders_manage_form_data['wcfm_om_discount']);
                            if (is_wp_error($result)) {
                                wcfm_log($result->get_error_message());
                                echo '{"status": false, "message": "' . $result->get_error_message() . '"}';
                                die;
                            }
                        } catch (Exception $e) {
                            wcfm_log($e->getMessage());
                            echo '{"status": false, "message": "Algo salio mal, por favor comuniquese con su administrador."}';
                            die;
                        }
                    // }//end if
                }//end if
                ...
                if (apply_filters('wcfm_allow_customer_billing_details', true)) {
                    ...
                    foreach ($wcfm_order_billing_fields as $wcfm_order_default_key => $wcfm_order_default_field) {
                        if (is_callable([ $order, "set_{$wcfm_order_default_key}" ])) {
                ...
                // Add Order Note
                if (wcfm_is_vendor()) {
                    // translators: 1) view order link 2) vendor store link 3) customer link
                    $wcfm_messages = sprintf(__('<b>%1$s</b> order added by <b>%2$s</b> for customer <b>%3$s</b>.', 'wc-frontend-manager-ultimate'), '#<a target="_blank" class="wcfm_dashboard_item_title" href="'.get_wcfm_view_order_url($new_order_id).'">'.$new_order_id.'</a>', $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor($vendor_id), ( $customer_id ) ? '<a target="_blank" class="wcfm_dashboard_item_title" href="'.get_wcfm_customers_details_url($customer_id).'">'.get_userdata($customer_id)->display_name.'</a>' : '');
                ...
                update_post_meta($order->get_id(), 'wcfm_invoice_number', $wcfm_orders_manage_form_data['wcfm_invoice_number']);
            ...
            if (! $has_error) {
                echo '{"status": true, "message": "'.$wcfm_orders_manage_messages['order_published'].'", "redirect": "'.get_wcfm_view_order_url($new_order_id).'"}';
            } else {
                echo '{"status": false, "message": "'.$wcfm_orders_manage_messages['order_failed'].'"}';
            }
        } else {
            echo '{"status": false, "message": "'.$wcfm_orders_manage_messages['no_product'].'"}';
        }//end if

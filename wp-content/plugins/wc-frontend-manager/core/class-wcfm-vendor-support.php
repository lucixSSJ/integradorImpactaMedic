		...
		if( $is_marketplace ) {
			if( !wcfm_is_vendor() || apply_filters( 'wcfm_is_allow_get_all_vendor_list_by_force', true ) ) {
				...
				if( $is_marketplace == 'wcpvendors' ) {
					...
					$array_search = [];
					if ( isset($vendor_search_data['start_date']) ) {
						$array_search[] = [
							'key'     => 'wcfm_membership_next_schedule',
							'value'   => $vendor_search_data['start_date'],
							'compare' => '>=',
						];
						unset($vendor_search_data['start_date']);
					}
					if ( isset($vendor_search_data['end_date']) ) {
						$array_search[] = [
							'key'     => 'wcfm_membership_next_schedule',
							'value'   => $vendor_search_data['end_date'],
							'compare' => '<=',
						];
						unset($vendor_search_data['end_date']);
					}
					if (isset($vendor_search_data['without_membership'])){
						$array_search[] = [
							'key'     => 'wcfm_membership',
							'compare' => 'NOT EXISTS',
						];
						unset($vendor_search_data['without_membership']);
					}

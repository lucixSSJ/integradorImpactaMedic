...
class WCFM_Vendors_Controller {
	...
	public function processing() {
		...
		$in_30_days = strtotime( '+30 days', $today_date );
		$in_15_days = strtotime( '+15 days', $today_date );

		if ( isset( $_POST['due_date'] ) && ! empty( $_POST['due_date'] ) ) {
			$due_date = wc_clean( $_POST['due_date'] );

			switch ( $due_date ) {
				case 'without_membership':
					$vendor_search_data['without_membership'] = true;
					break;
				case 'already_due':
					$vendor_search_data['end_date'] = $today_date;
					break;
				case 'in_15_days':
					$vendor_search_data['start_date'] = $today_date;
					$vendor_search_data['end_date'] = $in_15_days;
					break;
				case 'in_30_days':
					$vendor_search_data['start_date'] = $in_15_days;
					$vendor_search_data['end_date'] = $in_30_days;
					break;
				case 'more_than_30_days':
					$vendor_search_data['start_date'] = $in_30_days;
					break;
			}
		}
					...
					if( $wcfm_membership && function_exists( 'wcfm_is_valid_membership' ) && wcfm_is_valid_membership( $wcfm_membership ) ) {
						...
						if( $next_schedule ) {
							...
							if( ( $is_free == 'no' ) && ( $subscription_type != 'one_time' ) ) {
								$custom_schedule_styles = '';
								if ( $next_schedule < $today_date ) {
									$custom_schedule_styles = 'color: #D01F3C';
								} elseif ( $next_schedule >= $today_date && $next_schedule <= $in_15_days ) {
									$custom_schedule_styles = 'color: #FF7400';
								} elseif ( $next_schedule > $in_15_days && $next_schedule <= $in_30_days ) {
									$custom_schedule_styles = 'color: #008C00';
								}
								
								$membership_label .= "<br />";
								$membership_label .= '<span class="wcfmfa fa-clock img_tip" style="'. $custom_schedule_styles . '" data-tip="' . __( 'Next payment on', 'wc-frontend-manager' ) . '"></span>&nbsp;';
								$membership_label .= '<span style="'. $custom_schedule_styles . '">' . date_i18n( wc_date_format(), $next_schedule ) .'</span>';
							}
